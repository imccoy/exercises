...
Theorem hoare_asgn_fwd_exists :
  ...
  {{fun st => exists m, P (t_update st X m) /\
                st X = aeval (t_update st X m) a }}.
Proof.
  intros functional_extensionality v a Q st' HQst' HaQ.

  inversion HQst'.
  exists (Q X). split.
  - rewrite t_update_shadow. simpl.
    rewrite t_update_same. assumption.
  - rewrite t_update_shadow. rewrite t_update_same.
    unfold t_update. unfold X. simpl. symmetry. assumption.
Qed.
...
Proof.
  apply hoare_if.
    - eapply hoare_consequence_pre. eapply hoare_asgn.
      ...
      inversion H. inversion H1. apply le_plus_minus. apply leb_complete. assumption.
    - eapply hoare_consequence_pre. eapply hoare_asgn.
...
Definition havoc_pre (X : id) (Q : Assertion) : Assertion :=
  (fun st => forall n, Q (t_update st X n)). (* this seems pretty heavyweight, but the following proof came out real nice so I guess it's right? *)
