...
Proof.
  ...
  Case "S". intros n Hn. simpl. rewrite Hn. reflexivity.
  Qed.
(** [] *)
...
byntree_ind :
  forall P : byntree -> Prop,
    ...
    (forall (y : yesno) (l r : byntree), P l -> P r -> P (nbranch y l r)) ->
...
tree_ind : forall (X:Type) (P : tree X -> Prop),
  (forall (x:X), P (leaf x)) ->
  (forall (l r:tree X), P l -> P r -> P node l r) ->
  (forall (t:tree X), P t).
*)
...
Proof.
  induction n.
  Case "0". unfold P_plus_assoc. simpl. reflexivity.
  Case "S". unfold P_plus_assoc. simpl.
    unfold P_plus_assoc in IHn.
    intros m p.  rewrite <- IHn. reflexivity.
Qed.
...
Proof.
  induction n.
  Case "0". unfold P_plus_comm. intros m. rewrite plus_0_r. reflexivity.
  Case "S".
    unfold P_plus_comm.
    intros m.
    unfold P_plus_comm in IHn.
 ...
 *)
Check and_ind.
(* my guess on that one was pretty badly wrong, eh? *)
(* it's actually
Inductive and (P Q : Prop) : Prop :=  conj : P -> Q -> P /\ Q
and_ind: forall P Q P0 : Prop, (P -> Q -> P0) -> P /\ Q -> P0
*)
(** [] *)
...
Print or.
(* or_ind: forall (P Q : Prop),
             (P -> P \/ Q) ->
             (Q -> P \/ Q) ->
             (P \/ Q). *)
Check or_ind.
(* it is clear that I do not actually understand what coq is doing here. It's actually:
or_ind : forall P Q P0 : Prop, (P -> P0) -> (Q -> P0) -> P \/ Q -> P0
*)
(** [] *)
...
(* Probably not, but I'll give it a whirl anyway.
False_ind : .

I guess it's just going to be empty?
*)
Check False_ind.
(* 
