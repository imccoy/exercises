...
Proof.
  ...
  - intros HbevalR. induction HbevalR.
    simpl. reflexivity.
    simpl. reflexivity.
...
Proof.
  set (S0 := (t_update empty_state X 2)).
  set (S1 := t_update S0 Y 0).
  set (S2 := t_update S1 Y 2).
  set (S3 := t_update S2 X 1).
  set (S4 := t_update S3 Y 3).
  set (S5 := t_update S4 X 0).
  apply E_Seq with S1.
  - (* assignment Y=0 *)
    apply E_Ass. reflexivity.
  - (* While 0 < X *)
...
Proof.
  ...
  inversion Heval. subst. apply t_update_eq.
Qed.
  ...
  - (* -> *)
    ...
    induction c.
      ...
      + (* Seq *)
        simpl in Hnowhiles.
        apply no_whiles_Seq.
        apply IHc1. apply andb_true_iff in Hnowhiles. inversion Hnowhiles. assumption.
        apply IHc2. apply andb_true_iff in Hnowhiles. inversion Hnowhiles. assumption.
      + (* If *)
        simpl in Hnowhiles.
        apply no_whiles_If.
        apply IHc1. apply andb_true_iff in Hnowhiles. inversion Hnowhiles. assumption.
...
Proof.
  ...
  induction noloop.
    - (* SKIP *) intros st. exists st. apply E_Skip.
    - (* ::= *) intros st. exists (t_update st i (aeval st a)). apply E_Ass. reflexivity.
    - (* ;; *) intros st.
      inversion Hnowhile.
...
Proof.
  ...
  generalize dependent st2.
  induction E1.
    -  (* E_Skip *) intros st2 s2 E2. inversion E2. subst. split. reflexivity. reflexivity.
    -  (* E_Break *) intros st2 s2 E2. inversion E2. subst. split. reflexivity. reflexivity.
    -  (* E_Ass *) intros st2 s2 E2. inversion E2.
      split. rewrite <- H5. rewrite <- H. reflexivity. reflexivity.
    -  (* E_SeqContinue *) intros st2 s2 E2. inversion E2.
      + (* "Continue" *) subst.
        assert (st' = st'0 /\ SContinue = SContinue) as EQ1.
        * (* proof of assertion *)
        apply IHE1_1. apply H1.
        *  inversion EQ1. subst st'.  apply IHE1_2. apply H5.
      +  (* Break *) apply IHE1_1 in H4. inversion H4. inversion H6.
    -  (* E_SeqBreak *) intros st2 s2 E2. inversion E2.
        * (* Break *) apply IHE1 in H1. inversion H1. inversion H7.
        * subst. split. apply IHE1 in H4. inversion H4. apply H.  reflexivity.
    -  (* E_IfTrue *) intros st2 s2 E2. destruct (beval st b) eqn:Hbeval.
      +  (* b true *) inversion E2. subst.
        apply IHE1 in H7. apply H7.
        subst. rewrite Hbeval in H6. inversion H6.
      +  (* b false *) inversion H.
    -  (* E_IfFalse *) intros st2 s2 E2. destruct (beval st b) eqn: Hbeval.
      +  (* b true *) inversion H.
      +  (* b false *) inversion E2.
        subst. rewrite H6 in Hbeval. inversion Hbeval.
        subst. apply IHE1 in H7. apply H7.
    -  (* E_WhileLoopContinue *) intros st2 s2 E2.
      inversion E2. subst.
      +  (* LoopContinue *)
    ...
    -  (* E_WhileLoopBreak *) intros st2 s2 E2.
      inversion E2.
      subst.
      +  (* LoopContinue *)
