...
find ::
  ...
  -> Optional a
find f =
  foldRight (\x found -> if f x then Full x else found) Empty
...
lengthGT4 = lengthGt 4
  where lengthGt 0 (_ :. _) = True
        lengthGt n (_ :. as) = lengthGt (n - 1) as
        lengthGt _ _ = False
...
-- to make something that doesn't act as a list reverser for _any_ inputs? Probably not
