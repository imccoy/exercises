...
mapParser ::
  ...
  -> Parser a
  -> Parser b
mapParser f (P a) = P $ \input -> case a input of
...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P pa) = P $ \input -> case pa input of
                                      ErrorResult e -> ErrorResult e
                                      Result input' a -> let (P pb) = f a
                                                          in pb input'
...
list ::
  Parser a
  -> Parser (List a)
list el = let present = flbindParser el $ \h ->
                        flbindParser (list el ||| valueParser Nil) $ \t ->
                        valueParser $ h :. t
           in present ||| valueParser Nil
...
natural ::
  Parser Int
natural = flbindParser (list1 digit) $ \digits ->
          case read digits of
...
sequenceParser = foldRight f (valueParser Nil)
  where f phead ptail = flbindParser phead $ \head ->
                         flbindParser ptail $ \tail ->
                         valueParser $ head :. tail 
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany n ps = sequenceParser $ replicate n ps
...
firstNameParser ::
  Parser Chars
firstNameParser = flbindParser upper $ \head ->
                  flbindParser (list lower) $ \tail ->
                  valueParser $ head :. tail
...
surnameParser ::
  Parser Chars
surnameParser = flbindParser upper $ \head ->
                flbindParser (thisMany 5 lower) $ \tail1 ->
                flbindParser (list lower) $ \tail2 ->
                valueParser $ head :. tail1 ++ tail2
...
phoneParser ::
  Parser Chars
phoneParser = flbindParser digit $ \head ->
              flbindParser phoneBodyParser $ \phoneBody ->
              flbindParser (is '#') $ \_ ->
              valueParser (head :. phoneBody)
...
personParser ::
  Parser Person
personParser =
  flbindParser ageParser $ \ageV ->
  flbindParser (list1 (is ' ')) $ \_ ->
  flbindParser firstNameParser $ \firstNameV ->
  flbindParser (list1 (is ' ')) $ \_ ->
  flbindParser surnameParser $ \surnameV ->
  flbindParser (list1 (is ' ')) $ \_ ->
  flbindParser smokerParser $ \smokerV ->
  flbindParser (list1 (is ' ')) $ \_ ->
  flbindParser phoneParser $ \phoneV ->
  valueParser $ Person ageV firstNameV surnameV smokerV phoneV
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  f <$> pv = bindParser (\v -> valueParser $ f v) pv
