...
jsonString = between (is '"') (is '"') (list jsonChar)
  where jsonChar = noneof "\\\"" ||| (is '\\' >>> ((is 'u' >>> hex) ||| specialChar))
        specialChar = do c <- character
                         case toSpecialCharacter c of
                           Full r -> valueParser $ fromSpecialCharacter r
                           Empty -> failed
...
jsonNumber ::
  Parser Rational
jsonNumber = do sign <- ((:. Nil) <$> is '-') ||| valueParser ""
                beforeDecimal <- digits1
                afterDecimal <- (is '.' >>= (\_ -> ('.' :.) <$> digits1)) ||| valueParser ""
                case readFloat $ sign ++ beforeDecimal ++ afterDecimal of
                  Full x -> pure x
                  _ -> failed
...
jsonObject ::
  Parser Assoc
jsonObject = betweenSepbyComma '{' '}' parseAssoc
  where parseAssoc = do key <- jsonString
                        spaces
                        charTok ':'
                        spaces
                        value <- jsonValue
