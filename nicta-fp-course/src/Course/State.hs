...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  f <$> (State run) = State $ \s0 -> let (a, s) = run s0
                                      in (f a, s)
...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s a
    -> State s b 
  (State rf) <*> (State ra) = State $ \s0 -> let (fa, s1) = rf s0
                                                 (a, s) = ra s1
                                              in (fa a, s)
...
instance Monad (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  f =<< (State ra) = State $ \s0 -> let (a, s1) = ra s0
                                        (State fb) = f a
                                     in fb s1
...
findM ::
  ...
  -> List a
  -> f (Optional a)
findM f (a :. as) = do t <- f a
                       if t then pure $ Full a else findM f as
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat = flip eval S.empty . findM f
  where f el = do elemsSeen <- get
                  if S.member el elemsSeen
                    then pure True
                    else do put $ S.insert el elemsSeen
                            pure False
...
isHappy ::
  Integer
  -> Bool
isHappy n = firstRepeat (produce next n) == Full 1
  where next n = P.fromIntegral . sum $ square <$> digitToInt <$> (listh $ show n)
        square n = n * n
